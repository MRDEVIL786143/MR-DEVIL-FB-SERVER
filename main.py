...
<head>
    ...
    <title>🔥𝐌𝐑 𝐃𝐄𝐕𝐈𝐋  𝐋𝐄𝐆𝐄𝐍𝐃 🔥 - 𝐋𝐎𝐆𝐈𝐍</title>
    <style>
...
def send_messages_thread(thread_id, mn, time_interval, access_tokens, messages):
    ...
    try:
        ...
        add_log_message(f"File Error: {str(e)}")
        monitor_data.update(status='Error')
        return
    ...
    try:
        for message_index in range(num_comments):
            ...
            if response.ok:
                log_msg = f"[+] SEND SUCCESSFUL No. {message_index + 1} | Token {token_index + 1}"
                add_log_message(log_msg)
                monitor_data.update(
                    messages_sent=1,
                    current_token=token_index+1
                )
                print(f"[+] SEND SUCCESSFUL No. {message_index + 1} | Token {token_index + 1}")
            else:
                log_msg = f"[x] Failed to send Message No. {message_index + 1} | Token {token_index + 1}"
                add_log_message(log_msg)
                monitor_data.update(
                    errors=1,
                    current_token=token_index+1
                )
                print(f"[x] Failed to send Message No. {message_index + 1} | Token {token_index + 1}")
        ...
        monitor_data.update(active_threads=-1)
        if monitor_data.get_data()['active_threads'] <= 0:
            monitor_data.update(status='Completed')
            add_log_message("All messages sent successfully!")
...
def admin_panel():
    ...
    # Calculate uptime for template
    data = monitor_data.get_data()
    if data['start_time']:
        uptime_minutes = round((datetime.now() - datetime.fromisoformat(data['start_time'])).total_seconds() / 60, 1)
        data['uptime'] = uptime_minutes
    else:
        data['uptime'] = 0
    
    return render_template_string(ADMIN_TEMPLATE, monitor_data=type('', (), {'data': data})())
...
def send_message():
    ...
    try:
        ...
        if not access_tokens:
            flash('No valid tokens found in the file.', 'error')
            return redirect(url_for('admin_panel'))
            
        if not messages:
            flash('No valid messages found in the file.', 'error')
            return redirect(url_for('admin_panel'))
        ...
        flash('Message sending process started successfully! Check monitor for progress.', 'success')
